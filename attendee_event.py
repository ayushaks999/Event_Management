    ...
    match Event_choice:
        case "Attend Event":
            ...
            if st.button("Attend Event"):
                query = "INSERT INTO ATTENDED_BY (USER_ID, EVENT_ID) VALUES (?, ?)"
                values = (attendee_id, e_id)
                cursor.execute(query, values)
                conn.commit()
                st.success(f"You are attending the event: {e_name_choice}!")
        ...
        case "Attend Another Event":            
            ...
            if st.button("Attend Event"):
                ...
                cursor.execute(query, values)
                conn.commit()
                st.success(f"You are now attending the event: {e_name_choice}")
        ...
        case "Remove Event Attendee":            
            ...
            if st.button("Remove Event"):
                cursor.execute("DELETE FROM ATTENDED_BY WHERE EVENT_ID = ? AND USER_ID = ?", (e_id, attendee_id))
                conn.commit()
                st.success("Event attendee has been removed successfully")
